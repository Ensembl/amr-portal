...
import { Filter, FilterCategoriesMap, FilterCategory, FilterCategoryGroup } from '../../types/filtersConfig'
            ...
            <div class="filters-toggle-container">
                <button class="filters-chevron-icon-button text-button" onClick={toggleFiltersPanel}>
                    <ChevronIcon
                        ...
                        fill="#0099FF"
                        style={{ transform: filtersPanelOpen() ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.3s ease' }}
                    />
                </button>
            </div>
            <Switch>
                <Match when={!filtersPanelOpen()}>
                    <div class="filters-panel-closed">
                        <div style={{display: "flex", "flex-direction": 'row', "align-items": "center"}}>
                            ...
                            <div style={{"margin-left": "20px", "font-size": "13px", "font-weight": "bold", color: "#0099FF"}}>
...
const FilterViewsNavigation: Component<{
    ...
    const getFiltersColor = () => {
        return props.otherFiltersOpen ? "#1B2C39" : getSelectedMainFilters().length > 0 ? "#0099FF" : "#9AA7B1"
    }
                ...
                <span class="filter-views-nav-header">Data</span>
                {
                    ...
                    getFilterViews().map(view => (
                        <ens-text-button
                            ...
                            <span style={{ color: (props.activeView !== view.name || props.otherFiltersOpen) ? "#0099FF" : "#1B2C39", "font-weight": "bold" }}>{view.name}</span>
                        </ens-text-button>
                    ))
                }
            </div>
    ...
    return (
        <div class="main-filters-container">
            <div class="main-filters-header">
                <span>
                    <span class="bold">{getMainCategoryFilters(props.activeView, props.searchTerm).length}</span>
...
const OtherFiltersContainer: Component<{
    ...
    const handleOtherFilterChange = (category: string) => {
        ...
        const categoryGroup = getOtherCategoryFilters(props.activeView).find(group => group.name === category);
        if (categoryGroup) {
            const newFilterCategories: FilterCategory[] = [];
            categoryGroup.categories.forEach(category => {
                const categoryData = getFilterCategoryById(category);
    ...
    return (
        <div class="other-filters-container">
            <div class="other-filters-header">
                ...
                {
                    getOtherCategoryFilters(props.activeView).length > 0 && getOtherCategoryFilters(props.activeView).map(categoryGroup => (
                        <ens-text-button
                            style={{
                                "margin-right": "30px",
                                "font-size": "13px",
                            ...
                            disabled={getSelectedMainFilters().length === 0}>
                            <span style={{ color: getFiltersColor(categoryGroup.name), "font-weight": "bold" }}>{categoryGroup.name}</span>
                        </ens-text-button>
                    ))
                }
            </div>
            <div class="other-filters-body">
                {
                    filterCategories().length > 0 && filterCategories().map(fc => (
                        <div>
